...
`skill.communityGet` returns dsh-owned detail for one immutable identity. The adapter combines the upstream skill detail, complete paginated version list up to a configurable count limit, exact-version metadata, file list, bounded raw `SKILL.md`, and exact `resolve` result, then rejects any namespace, slug, or version disagreement. An example prompt is present only when the parsed release metadata contains a string at `metadata.examplePrompt`. The official local command is the exact `skillhub install <slug> --namespace <namespace> --version <version>` form, and every interpolated token must match the CLI token grammar.

Exact artifact bytes use the Host-only `GET /api/skill.download` surface instead of JSON-RPC. The browser hands one same-origin GET to its native download manager, preserving streaming without retaining the archive in browser memory. The Host resolves the requested version, forwards the archive while decoding it, and fails the stream unless every bounded central entry references the verified local header and every file size and SHA-256 matches the exact release file list. The response carries a sanitized canonical-name/version attachment filename. Browsing and downloading never write an installation receipt or register the release.
