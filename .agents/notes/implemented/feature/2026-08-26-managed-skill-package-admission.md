...
Host installation code admits exact releases through one typed module and can distinguish policy rejection, integrity failure, identity conflict, corrupt storage, and commit failure. Tests cover root and wrapped packages, portable path attacks, exact and excessive archive limits, manifest and fingerprint drift, shared skill parsing, cancellation, atomic failure, durable receipt and content corruption, idempotent retry, package recovery, and canonical-name ownership.

This decision intentionally stops at package admission. The [exact install lifecycle](2026-08-27-managed-skill-exact-install-lifecycle.md) owns resolver-driven install operations, idempotency records, and startup reconciliation. Web controls, callable provider contribution, update, disable, uninstall, and crash-durable `fsync` policy remain outside admission. Filesystem modes provide practical accidental-write protection on POSIX systems, not a security barrier against the owning operating-system account.
