...
Dynamic Cordis also reserves bare `cordis` for runtime identifiers: `cordis/*` event names, the `cordis` UI locale and input-source namespaces, and the catalog scope that routes those events to the extensions documentation. Files containing both these identifiers and framework package references are exempt from the generic pass; `EXACT_EDITS` pins every co-located `@deepseek-ai/cordis` import and `declare module` target so the exemption cannot hide an unscoped package reference.
