...
[CI](../../../../.github/workflows/ci.yml) selects `ubuntu-latest` and `windows-latest` for pull requests when `github.repository` is not `deepseek-harness/deepseek-harness`. The copied-repository Linux jobs use lower worker limits appropriate for standard GitHub-hosted runners, including serial snapshot replay so concurrent checks cannot starve streaming fixtures and trigger provider retries. A copied repository with the standard Windows runner labels can select its self-hosted pool through `DSH_CI_FAILOVER_WINDOWS=selfhosted`; the [copied-repository Windows failover note](2026-08-26-copied-repository-windows-failover.md) owns that opt-in. The canonical repository retains its organization-owned default runners and both self-hosted failover paths.
