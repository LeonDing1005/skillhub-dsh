...
Dynamic Cordis 也把裸 `cordis` 保留为运行时标识：`cordis/*` 事件名、`cordis` UI locale 与 input-source namespace，以及把这些事件路由到 extensions 文档的 catalog scope。同时包含这些标识与框架包引用的文件不参与通用改写；`EXACT_EDITS` 钉住其中每一处 `@deepseek-ai/cordis` import 与 `declare module` 目标，避免豁免掩盖未加 scope 的包引用。
