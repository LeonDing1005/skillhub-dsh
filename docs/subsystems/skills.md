...
`ManagedSkillStore` is a Host-internal library, not a Cordis service or skill provider. It accepts one exact resolved Community Skill release and complete ZIP bytes, validates the archive in a unique private staging directory, and publishes `content/` with `receipt.json` through one directory rename. `ManagedInstallationService` adds the first lifecycle operation above that store: installing one exact release through a Host-supplied resolver, operation record, and idempotency key. Neither class makes admitted content callable; a later managed provider owns that transition.
...
Installation operation records live under `v1/operations/` and remain separate from package receipts. A running record reserves one caller-supplied idempotency key before the resolver reacquires the artifact, and an owner-pid target lock rejects same-target installs from another service instance while the owner is alive. A completed record points at the committed package target after the receipt has verified. Repeating the same key for the same target replays the completed result after restart without contacting the Registry Instance; using that key for another target fails as an idempotency-key conflict. A second live operation for the same target with a different key fails as in progress.

Startup recovery creates the private roots, deletes abandoned staging and `.admitting-*` directories, validates complete receipts and immutable content, removes stale running operation records and dead target locks, and keeps only completed operation records whose receipts still verify. Recovery never promotes partial package data. Install results expose a receipt projection without the source server or managed content path.
