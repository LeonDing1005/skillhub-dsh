...
逐会话 UI 状态中的选择与活跃视图位于已声明的聊天 store（`stores.ts` `createChatStore`）中；InputHub 拥有输入区状态机，并将草稿镜像到该 store 以便持久化。在普通 Session scope 上，`ctx.conversation.insertSkillToken(name)` 会通过 ui-input-trigger 的纯文本事件，用受空白边界保护的 `/name ` token 替换已保存的 textarea selection，然后把焦点放回 composer 的结果光标处，但不提交；bar 尚未挂载时产生的聚焦请求会保留到下次挂载。该操作拒绝 subagent scope。apply 将同一个 store handle 传给严格限定于会话的子树、聊天视图和详情注册，因此每个会话内共享一个实例，框架拥有其生命周期。组件保持纯粹：框架标准工具包提供 `useSession`／`sessionId`、全局 `useSessions`／`useWorkspaces`，以及输入状态机的 `useInput`／`inputActions`；store 表层与 inject factory 提供其余状态和回调。
