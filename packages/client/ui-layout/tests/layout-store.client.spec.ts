...
describe('createLayoutStore', () => {
  it('initializes the sidebar at its default width, details closed, wide viewport assumed', () => {
    ...
    expect(store.getSnapshot()).toEqual({ sidebar: SIDEBAR_DEFAULT, details: 0, narrow: false, narrowExpanded: false, page: undefined })
  })
  ...
  it('narrow toggleSidebar flips only the re-expand override; the width preference survives', () => {
    ...
    actions.toggleSidebar()
    expect(store.getSnapshot()).toEqual({ sidebar: 400, details: 0, narrow: true, narrowExpanded: true, page: undefined })
    actions.toggleSidebar()
  ...
  it('switches the center between a named page and the conversation', () => {
    const { store, actions } = createLayoutStore().create()
    actions.openPage(shellPageId('skill-center'))
    expect(store.getSnapshot().page).toBe('skill-center')
    actions.showConversation()
    expect(store.getSnapshot().page).toBeUndefined()
  })
