...
原生技能中心始终存在；Host 同时设置 `DSH_SKILLHUB_BASE_URL` 和 `DSH_SKILLHUB_REGISTRY_INSTANCE_ID` 后，其社区目录才会启用。这两个独立设置分别标识一个上游端点和一个稳定的注册表实例。它们以及上游凭据、上游响应类型和 Host 路径都不会进入浏览器。
