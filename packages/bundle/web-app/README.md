...
The native Skill Center is always present; its Community catalog activates when both `DSH_SKILLHUB_BASE_URL` and `DSH_SKILLHUB_REGISTRY_INSTANCE_ID` are set on the Host. These independent settings identify one upstream endpoint and one stable registry instance. Neither value, upstream credentials, upstream response types, nor Host paths cross into the browser.
