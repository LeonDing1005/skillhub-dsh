...
`parseSkillDocument(raw)` exposes these parsing and normalization rules to trusted same-process callers that need to validate a skill without registering a filesystem provider. It returns the parsed metadata, invocation policy, and Markdown body, and throws the same normalized parse errors used by filesystem discovery.
