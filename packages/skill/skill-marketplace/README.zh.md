...
| `versionCountLimit` | `1000` | 加载一个精确发布版本详情时可接受的最大发布版本数。 |
| `zipDirectoryMaxBytes` | `4194304` | 核验精确发布 ZIP 时保留的最大中央目录字节数。 |
...
`get(identity)` 通过组合 SkillHub 的详情、不超过 `versionCountLimit` 的全部版本分页、精确版本元数据、文件、有字节上限的原始 `SKILL.md` 和 `resolve` 响应来检查一个精确发布版本。全部返回值均由 dsh 所有，每个响应的标识都必须一致，CLI 命令 token 必须是安全标识，并且只有字符串类型的 `metadata.examplePrompt` 才会成为 `examplePrompt`。`download(identity)` 再次核验精确版本元数据和 resolve 结果，并在流式传递精确归档时核验每个中央目录条目及其本地文件头引用，以及每个已记录文件的大小和 SHA-256。中央目录保留量受 `zipDirectoryMaxBytes` 限制。Host 用规范技能名称和版本命名响应。两种操作都不会安装或注册该发布版本。
