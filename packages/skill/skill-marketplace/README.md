...
`get(identity)` inspects one exact release by combining SkillHub detail, every version page, exact-version metadata, files, byte-bounded raw `SKILL.md`, and `resolve` responses. Every returned value is dsh-owned, every response identity must match, CLI command tokens must be safe identifiers, and `examplePrompt` is present only for a string `metadata.examplePrompt`. `download(identity)` repeats exact-version metadata and resolve verification, verifies the ZIP signature, then returns the exact artifact as a stream named from the canonical skill name and version. Neither operation installs or registers the release.
