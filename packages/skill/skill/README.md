...
- `ctx.skills.inventory({ cwd?, signal?, scope? })` Returns every discovered provider candidate, including names shadowed by a nearer scope or higher-priority provider. This host-only diagnostic view retains provider, source, and local path facts for a safe projection; it is not a model-facing catalog.
- `ctx.skills.candidates({ cwd?, signal?, scope? })` Returns the winning candidates with provider and path facts retained for host projections that need to distinguish resolution from installation.
...
- **Duplicate resolution is first-wins** — later lower-priority candidates within a layer are logged and hidden from operational catalogs, and a nearer layer shadows a farther one silently; `inventory()` is the host-only API for inspecting every candidate.
